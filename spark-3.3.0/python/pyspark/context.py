...
class SparkContext:
    ...
    def install_pypi_package(self, pypi_package, repo=None):
        """
        ...
        if self._is_package_installed(pypi_package):
            raise ValueError("Package already installed for current Spark context!")
        if repo is not None:
            repo = self._validate_repository(repo)

        def _run_pip(pypi_package, repo, iterator):
            import sys
            from pyspark.util import CommandUtils
            if repo is None:
                return CommandUtils.run_command(sys.executable + " -m pip install " + pypi_package)
            else:
                return CommandUtils.run_command(sys.executable + " -m pip install " +
                    pypi_package + " --index-url " + repo)
        ...
        if _run_pip(pypi_package, repo, None) != 0:
            return
        self._add_package_descriptor(pypi_package, repo)
        ...
        if num_executors > 0:
            ...
            dummyRDD.foreachPartition(functools.partial(_run_pip, pypi_package, repo))
    ...
    def uninstall_package(self, package):
        ...
        if _run_pip(package, None) != 0:
            return
        self._remove_package_descriptor(package)
    ...
    def list_packages(self):
        """
        List python packages available for current Spark session
        """
        if self._conf.get("spark.pyspark.virtualenv.enabled") != "true":
    ...
    def _validate_package(self, package):
        ...
        if not self._is_valid_package_and_version(package):
    ...
    def _validate_repository(self, repo):
        import six
        if not isinstance(repo, six.string_types):
            raise ValueError("Invalid repository URL: Repository URL must be a string")
        repo = repo.strip()
        if not self._is_valid_url(repo):
            raise ValueError("Invalid repository URL: Repository URL must conform to the syntax "
                             "of a generic URI and must not contain comma")
        return repo

    def _build_package_descriptor(self, package, repo=None):
        package_descriptor = package
        if repo is not None:
            package_descriptor += "@" + repo
        return package_descriptor

    def _get_package_from_descriptor(self, package_descriptor):
        if package_descriptor:
            package_version_and_repo = package_descriptor.split("@")
            if package_version_and_repo:
                package_and_version = package_version_and_repo[0].split("==")
                if package_and_version:
                    return package_and_version[0]
        return None

    def _add_package_descriptor(self, package, repo=None):
        packages = self._conf.get("spark.pyspark.virtualenv.packages")
        package_descriptor = self._build_package_descriptor(package, repo)
        if packages:
            self._conf.set("spark.pyspark.virtualenv.packages", packages + "," + package_descriptor)
        else:
            self._conf.set("spark.pyspark.virtualenv.packages", package_descriptor)

    def _remove_package_descriptor(self, package):
        new_package_descriptors = []
        existing_package_descriptors = self._get_existing_package_descriptors()
        if existing_package_descriptors:
            for package_descriptor in existing_package_descriptors:
                existing_package = self._get_package_from_descriptor(package_descriptor)
                if existing_package and existing_package != package:
                    new_package_descriptors.append(package_descriptor)
            self._conf.set("spark.pyspark.virtualenv.packages", ",".join(new_package_descriptors))

    def _get_existing_package_descriptors(self):
        packages = self._conf.get("spark.pyspark.virtualenv.packages")
        if packages:
    ...
    def _is_package_installed(self, package):
        ...
        if existing_package_descriptors:
            for package_descriptor in existing_package_descriptors:
                existing_package = self._get_package_from_descriptor(package_descriptor)
                given_package = self._get_package_from_descriptor(package)
                if existing_package and existing_package == given_package:
                    return True
        return False
    ...
    def _is_valid_package_and_version(self, package):
        return self._matches(r'^[a-zA-Z0-9._=\-]+$', package)
