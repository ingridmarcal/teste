...
class UDFTests(ReusedSQLTestCase):
    ...
    def test_executing_in_uninterruptible_threads(self):
        jdf = self.sc._jvm.PythonSQLUtils.isRunningInUninterruptibleThreadDataFrame(
            self.spark._jsparkSession)
        df = DataFrame(jdf, self.spark)

        [row1] = df.collect()
        self.assertEqual(row1[0], True)

        self.spark.catalog.registerFunction(
            "booleanToString", UserDefinedFunction(lambda x: u'true' if x else u'false'))

        # In order to evaluate the Python UDF on each row, Spark tasks spin up separate threads
        # for evaluating the inputs to the python UDF; theses threads should extend
        # UninterruptibleThread. If this is not the case, then False will be given as input to the
        # Python UDF below, and the test will subsequently fail.
        [row2] = df.selectExpr("booleanToString(value)").collect()
        self.assertEqual(row2[0], u'true')
