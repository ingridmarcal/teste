...
class RDDTests(ReusedPySparkTestCase):
    ...
    def test_executing_in_uninterruptible_threads(self):
        jrdd = self.sc._jvm.PythonUtils.isRunningInUninterruptibleThreadRDD(self.sc._jsc)
        rdd = RDD(jrdd, self.sc)

        self.assertEqual([True], rdd.collect())

        # In order to evaluate the python function on each RDD element, Spark tasks spin up
        # separate threads for evaluating the inputs to the python function; these threads should
        # extend UninterruptibleThread. If it is not the case, then False will be given as input to
        # the Python function below, and the test will subsequently fail.
        transformedRdd = rdd.map(lambda x: u'true' if x else u'false')
        self.assertEqual([u'true'], transformedRdd.collect())
